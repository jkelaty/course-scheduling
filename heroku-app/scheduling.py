courses = None
profCoursePreference = dict()
profTimePreference = dict()
root = None
schedule = None
CoursesNames = None
...
def run(ifileS, pfileS):
    global courses
    global profCoursePreference
    global profTimepreference
    global root
    global schedule
    global CoursesNames

    ifile = StringIO(ifileS)
    pfile = StringIO(pfileS)

    # assuming that each credit is a 50-min class 
    creditDuration = 50
        
    #loading the data from the input file
    #ifile = input("Enter the data filename: ")
    ...
    #pfile = input("Enter the soft preferences filename: ")
    preferenceData = pd.read_csv(pfile, header=None)
...
#okay = run('largeDataSet.csv', 'preferences.csv')
#print(okay)
